...
def insertar_disponibilidad_prenda(id_prenda, id_talla, precio, stock):
    ...
    with conexion.cursor() as cursor:
        cursor.execute("INSERT INTO disponibilidad_prenda (id_prenda, id_talla_prenda, precio, stock) VALUES (%s, %s, %s, %s)",
                        (id_prenda, id_talla, precio, stock,))
    conexion.commit()
    conexion.close()


def disponibilidad_existe(id_prenda, id_talla_prenda):
    conexion = obtener_conexion()
    with conexion.cursor() as cursor:
...
def prenda_talla_existen(id_prenda, id_talla_prenda):
    conexion = obtener_conexion()
    with conexion.cursor() as cursor:
        cursor.execute("SELECT 1 FROM prenda WHERE id_prenda = %s", (id_prenda,))
        prenda_existe = cursor.fetchone() is not None

        cursor.execute("SELECT 1 FROM talla_prenda WHERE id_talla_prenda = %s", (id_talla_prenda,))
        talla_existe = cursor.fetchone() is not None

    conexion.close()
    return prenda_existe and talla_existe
...
def actualizar_disponibilidad_prenda(precio, stock, id_prenda, id_talla):
    conexion = obtener_conexion()
    with conexion.cursor() as cursor:
        cursor.execute("UPDATE disponibilidad_prenda SET precio = %s, stock = %s WHERE id_prenda = %s and id_talla_prenda = %s",
